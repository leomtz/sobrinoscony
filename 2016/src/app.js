...
var promisePoints = d3.json("/pairs2016.json");
var promiseStateInfo = d3.json("/estados2016.json");
...
promiseAll.then(function (values) {
    ...
    function updateSquare(kind, legends) {
        ...
        //Clear old legends
        //Get refs again
        var miniCanvas = d3.select(".minicanvas");
        ...
        var minis = enterLegends.append("rect")
            ...
            .attr('width', 12)
            .style('stroke-width', '0')
            .style('fill', function (d, i) { return colors[(2 * i + 5) % 7]; });
        ...
        if (kind === "grado_id" || kind === "s_year_id" || kind === "e_year_id" || kind === "area_id") {
            ...
            squares.transition()
                .duration(1000)
                .style('fill', function (d) { return colors[(2 * d[(kind)] + 5) % 7]; })
    ...
    //Button listening
    d3.select('#todos').on("click", function (r) { return updateSquare("x", legendsIndex); });
    d3.select('#porArea').on("click", function (r) { return updateSquare("area_id", legendsArea); });
    d3.select('#porGrado').on("click", function (r) { return updateSquare("grado_id", legendsGrado); });
    d3.select('#porInicio').on("click", function (r) { return updateSquare("s_year_id", legendsInicio); });
    d3.select('#porFin').on("click", function (r) { return updateSquare("e_year_id", legendsFin); });
    d3.select('#porEstado').on("click", function (r) { return updateMap(); });
