...
d3.json<Array<pPoints>>("/pairs.json").then(function (data){
    ...
    d3.select('#porGrado').on("click",  function () {
        ...
        rects.transition()
            .duration(1000)
            .attr('transform', d=>`translate(${offSetX(d.grado_id)-50}, 0)`)
            .style('fill',d=>colors[d.grado_id])
        })
    ...
    d3.select('#porArea').on("click",  function () {
        ...
        enterLegends.append("text")
            .text(d=>d.leg)
            .attr("x", d=>d.x+20)
            .attr("y", (d,i)=>30*(i%2) + 12)
            .classed('label',true)

        rects.transition()
            .duration(2500)
            .ease(d3.easeElastic)
            .attr('transform', d=>`translate(${offSetX(d.area_id)}, 0)`)
            .style('fill',d=>colors[d.area_id])
    ...
    d3.select('#porInicio').on("click",  function () {
        ...
        rects.transition()
            .duration(1000)
            .attr('transform', d=>`translate(${offSetX(d.s_year_id)}, 0)`)
            .style('fill',d=>colors[d.s_year_id])
    ...
    d3.select('#porFin').on("click",  function () {
        ...
        rects.transition()
            .duration(1000)
            .attr('transform', d=>`translate(${offSetX(d.e_year_id)}, 0)`)
            .style('fill',d=>colors[d.e_year_id])
